...
export default function App() {
  ...
  const generateProblem = async () => {
    const chatResponse = await client.chat.complete({
          ...
          messages: [{role: "user", content: "Génère un problème simple de mathématiques pour enfant et donne la réponse."}],
    })

      const generatedText = chatResponse.choices[0].message.content;
      console.log(generatedText.split("Réponse :"))
      const [question, answer] = generatedText.split("Réponse :");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-red-500 to-white p-6 text-center">
      ...
      <div className="bg-white p-4 rounded-xl shadow-lg max-w-lg mx-auto">
        ...
        <button onClick={generateProblem} className="bg-green-500 text-white px-4 py-2 rounded mt-2">Nouvelle Question</button>
      </div>
    </div>
  );
}